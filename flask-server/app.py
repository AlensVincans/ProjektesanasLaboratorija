from flask import Flask, request, jsonify, session, redirect, has_request_context
...
FRONTEND_ORIGIN = os.getenv("FRONTEND_ORIGIN", "https://lab.aleksandrsbelkins.info")
CORS(app, supports_credentials=True, resources={r"/*": {"origins": FRONTEND_ORIGIN}})

logging.basicConfig(level=logging.DEBUG)

ACTIVITY_MULTIPLIERS = {"sedentary": 1.15, "low": 1.2, "moderate": 1.5, "active": 1.75}
...
def load_products_from_db():
    ...
    for row in rows:
        ...
        name_no_diacritics = "".join(
            c
            for c in unicodedata.normalize("NFKD", name_normalized)
            if not unicodedata.combining(c)
        )
        blocked = {"dienas norma", "kombinetie tauki", "cukas tauki"}
        if (
            name_normalized in blocked
            or name_no_diacritics in blocked
            or not name_normalized
        ):
            continue
        foods.append(food)
...
def calculate_tdee():
    ...
    try:
        ...
        if weight <= 0 or height <= 0 or age <= 0:
            return jsonify(
                {"error": "Invalid input: weight, height, and age must be positive"}
            ), 400
    except (TypeError, ValueError):
        return jsonify(
            {"error": "Invalid input: weight, height, and age must be numeric"}
        ), 400
...
def optimize_diet():
    ...
    for food in foods:
        ...
        food_allergens_norm = "".join(
            c
            for c in unicodedata.normalize(
                "NFKD", (food.get("allergens") or "").lower()
            )
            if not unicodedata.combining(c)
        )
    ...
    x_vars = {
        name_to_var[f["name"]]: LpVariable(
            name_to_var[f["name"]],
            lowBound=0,
            upBound=min(3 * period_days, per_product_weekly_cap_units),
        )
        for f in available_foods
    }
    ...
    price_term = lpSum(
        f["price_per_100g"] * x_by_name[f["name"]] for f in available_foods
    )

    # Optional variety to avoid identical optimal sets across runs
    variety = bool(data.get("variety", True))
    variety_strength = float(
        data.get("variety_strength", 0.15)
    )  # noise weight per 100g unit for variety
    seed = data.get("seed")
    rng = random.Random(seed) if seed is not None else random.Random(os.urandom(8))
    noise_term = 0
    if variety and available_foods:
        noise_weights = {
            f["name"]: rng.uniform(-variety_strength, variety_strength)
            for f in available_foods
        }
        noise_term = lpSum(
            noise_weights[f["name"]] * x_by_name[f["name"]] for f in available_foods
        )

    for nut in nut_keys:
        model += (
            lpSum(f[nut] * x_by_name[f["name"]] for f in available_foods) >= norms[nut],
            f"Min_{nut}",
        )
    for nut in ["protein", "fat", "carbs", "kcal", "kj"]:
        if nut in norms_upper:
            model += (
                lpSum(f[nut] * x_by_name[f["name"]] for f in available_foods)
                <= norms_upper[nut],
                f"Max_{nut}",
            )
    ...
    for n in x_by_name.keys():
        norm = "".join(
            c
            for c in unicodedata.normalize("NFKD", n.lower())
            if not unicodedata.combining(c)
        )
        if "cukurs" in norm:
    ...
    def in_names(keyword_list):
        ...
        return [
            name
            for name in names
            if any(kw in normalized_name_map[name] for kw in keyword_list)
        ]

    oils = in_names(["ella"])
    refined_grains = in_names(["makaroni", "mannas", "milti", "maize"])
    whole_grains = in_names(
        ["griki", "auzu", "rudzu", "miezu", "putraimi", "risi", "grubas"]
    )
    ...
    nudge_penalty = (
        lpSum(0.01 * x_by_name[n] for n in (sweets + refined_grains))
        if (sweets or refined_grains)
        else 0
    )
    nudge_reward = (
        lpSum(-0.005 * x_by_name[n] for n in (vegetables + whole_grains + legumes))
        if (vegetables or whole_grains or legumes)
        else 0
    )
    model += (
        price_term + noise_term + nudge_penalty + nudge_reward,
        "Total_Cost_With_Health_Nudges_Variety",
    )

    # Oils: ≤30g/day (WHO) → 210g/week → 2.1 units/week
    if oils:
        model += (
            lpSum(x_by_name[name] for name in oils) <= (0.3 * period_days),
            "Max_Oil",
        )
    ...
    if vegetables:
        model += (
            lpSum(x_by_name[name] for name in vegetables) >= (4.0 * period_days),
            "Min_Vegetables",
        )
    ...
    if fruits:
        model += (
            lpSum(x_by_name[name] for name in fruits) >= (2.0 * period_days),
            "Min_Fruits",
        )
    ...
    if legumes:
        if period_days == 7:
            model += (
                lpSum(x_by_name[name] for name in legumes) >= 6.0,
                "Min_Legumes_Week",
            )
        else:
            model += (
                lpSum(x_by_name[name] for name in legumes) >= 1.0,
                "Min_Legumes_Day",
            )
    ...
    if refined_grains:
        model += (
            lpSum(x_by_name[name] for name in refined_grains) <= (2.0 * period_days),
            "Max_Refined_Grains",
        )
    ...
    if whole_grains and refined_grains:
        model += (
            lpSum(x_by_name[name] for name in whole_grains)
            >= lpSum(x_by_name[name] for name in refined_grains),
            "Min_Whole_vs_Refined",
        )
    ...
    if red_meat:
        if period_days == 7:
            model += (
                lpSum(x_by_name[name] for name in red_meat) <= 5.0,
                "Max_Red_Meat_Week",
            )
        else:
            model += (
                lpSum(x_by_name[name] for name in red_meat) <= 0.7,
                "Max_Red_Meat_Day",
            )
    ...
    if processed_meat:
        model += (
            lpSum(x_by_name[name] for name in processed_meat) == 0.0,
            "No_Processed_Meat",
        )
    ...
    if sweets:
        model += (
            lpSum(x_by_name[name] for name in sweets) <= (1.0 * period_days),
            "Max_Sweets",
        )
    ...
    if potatoes:
        if period_days == 7:
            model += (
                lpSum(x_by_name[name] for name in potatoes) <= 7.0,
                "Max_Potatoes_Week",
            )
        else:
            model += (
                lpSum(x_by_name[name] for name in potatoes) <= 1.0,
                "Max_Potatoes_Day",
            )
    if pasta:
        if period_days == 7:
    ...
    if LpStatus[status] != "Optimal":
        ...
        if LpStatus[status] == "Infeasible":
            ...
            for nut in nut_keys:
                total = sum(
                    f[nut] * value(x_by_name[f["name"]]) for f in available_foods
                )
                if total < norms[nut]:
    ...
    nutrient_totals = {
        nut: round(
            sum(f[nut] * value(x_by_name[f["name"]]) for f in available_foods), 2
        )
        for nut in nut_keys
    }
...
def oauth_login():
    ...
    redirect_uri = os.getenv("GOOGLE_REDIRECT_URI", "https://lab.aleksandrsbelkins.info/api/callback")
...
def chatgpt_endpoint():
    ...
    try:
        ...
        return jsonify(
            {"error": "ChatGPT API timeout. Please try again.", "success": False}
        ), 500
    except requests.exceptions.RequestException as e:
